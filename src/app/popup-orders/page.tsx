...
function PopupOrdersContent() {
  ...
  const startEdit = (order: PopupOrder) => {
          ...
          <div key={order.id} className={`rounded-2xl border ${slipSelectedIds.has(order.id) ? 'border-purple-500 ring-1 ring-purple-500/40' : 'border-gray-800'} bg-gray-950 p-5 space-y-3`}>
            ...
            {editingOrderId === order.id ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                <label className="flex flex-col gap-1"><span className="text-gray-400">Custom Letters:</span><input className="bg-gray-900 border border-gray-700 rounded px-2 py-1 text-white text-sm" value={editFields.text || ''} onChange={(e) => setEditFields((f) => ({ ...f, text: e.target.value }))} /></label>
                ...
                <label className="flex flex-col gap-1"><span className="text-gray-400">Phone:</span><input className="bg-gray-900 border border-gray-700 rounded px-2 py-1 text-white text-sm" value={editFields.phoneNumber || ''} onChange={(e) => setEditFields((f) => ({ ...f, phoneNumber: e.target.value }))} /></label>
